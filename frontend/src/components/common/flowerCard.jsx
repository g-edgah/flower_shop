...
const FlowerCard = ({id, name, type, price, image, liked, carted, userRefetch}) => {
    ...
    return (
        <div className="relative group h-73 md:h-91 w-42 min-w-42 md:w-55 md:min-w-45 bg-cartCard rounded-lg flex flex-col space-y-2 md:space-y-3">
            <ImHeart onClick={handleWishlistToggle} className={` absolute top-2 right-2   cursor-pointer size-7 ${liked ? 'text-summaryButtons block hover:text-gray-500' : 'text-gray-500 hidden group-hover:block hover:text-summaryButtons'}`} />
